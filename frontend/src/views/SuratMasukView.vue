<template>
  <div>
    ...
    <Modal :show="showModal" :title="editId ? 'Edit Surat Masuk' : 'Tambah Surat Masuk'" @close="showModal = false">
      <form @submit.prevent="handleSave" class="space-y-4">
        ...
        <div>
          <label class="label-cyber">Tanggal Terima *</label>
          <input type="date" v-model="form.tanggal_terima" class="cyber-input" required />
        </div>
        <div>
          <label class="label-cyber">Pengirim *</label>
          <input v-model="form.pengirim" class="cyber-input" required />
        </div>
        <div>
...
const form = ref({ nomor_surat: '', tanggal_surat: '', tanggal_terima: '', pengirim: '', perihal: '', keterangan: '' })

async function fetchData(page = 1) {
  loading.value = true
  try {
    const params = { page, search: search.value, tanggal_dari: filterDari.value, tanggal_sampai: filterSampai.value }
    const { data: res } = await api.get('/surat-masuk', { params })
    data.value = res.data
...
function openModal(row = null) {
  if (row) {
    editId.value = row.id
    form.value = { nomor_surat: row.nomor_surat, tanggal_surat: row.tanggal_surat?.split('T')[0], tanggal_terima: row.tanggal_terima?.split('T')[0], pengirim: row.pengirim, perihal: row.perihal, keterangan: row.keterangan || '' }
  } else {
    editId.value = null
    form.value = { nomor_surat: '', tanggal_surat: '', tanggal_terima: new Date().toISOString().split('T')[0], pengirim: '', perihal: '', keterangan: '' }
  }
...
async function handleSave() {
  ...
  try {
    ...
    if (editId.value) {
      await api.put(`/surat-masuk/${editId.value}`, fd, { headers: { 'Content-Type': 'multipart/form-data' } })
      Swal.fire('Berhasil', 'Surat masuk berhasil diupdate', 'success')
    } else {
...
async function handleDelete(row) {
  ...
  if (confirm.isConfirmed) {
    try { await api.delete(`/surat-masuk/${row.id}`); Swal.fire('Berhasil', 'Surat berhasil dihapus', 'success'); fetchData() }
