<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      <h1 class="font-heading text-xl text-cyber-primary tracking-wider">DATA SISWA</h1>
      ...
      <DataTable :columns="columns" :data="data" :loading="loading" :pagination="pagination" @page-change="fetchData($event)">
        ...
        <template #actions="{ row }">
          <div class="flex gap-2">
            <button v-if="canCreate" @click="openModal(row)" class="text-cyber-primary hover:text-cyber-primary/80 text-xs">Edit</button>
            <button v-if="canCreate" @click="handleDelete(row)" class="text-cyber-danger hover:text-cyber-danger/80 text-xs">Hapus</button>
          </div>
        </template>
      </DataTable>
    </div>
